...
describe('TranscriptViewPolicy', () => {
  ...
  it('adopts Host state, reads normal as Standard, and ignores identical writes', () => {
    ...
    host.publish({ status: 'ready', value: { linkOpening: 'sidebar', transcriptView: 'normal', performanceUsage: 'detailed', reasoningPreviewLines: 1 }, revision: 1, writable: true })
    expect(policy.mode.getSnapshot()).toBe('standard')
    policy.setMode('standard')
    expect(host.set).not.toHaveBeenCalled()

    host.publish({ value: { linkOpening: 'sidebar', transcriptView: 'compact', performanceUsage: 'detailed', reasoningPreviewLines: 1 }, revision: 2 })
    expect(policy.mode.getSnapshot()).toBe('compact')
  })

  it('adopts an accepted section standing at construction', () => {
    const host = stubConfigForm<ChatSettings>()
    host.publish({ status: 'ready', value: { linkOpening: 'sidebar', transcriptView: 'expanded', performanceUsage: 'detailed', reasoningPreviewLines: 1 }, revision: 1, writable: true })
  ...
  it.each(['compact', 'standard', 'detailed', 'verbose'] as const)('preserves an explicit %s setting without migration writes', (mode) => {
    const host = stubConfigForm<ChatSettings>()
    host.publish({ status: 'ready', value: { linkOpening: 'sidebar', transcriptView: mode, performanceUsage: 'detailed', reasoningPreviewLines: 1 }, revision: 1, writable: true })
    const policy = new TranscriptViewPolicy(host.scope)
