...
describe('Chat apply wiring', () => {
  ...
  it('contributes Chat View, node renderers, and stats', async () => {
    ...
    expect(b.runtime.slots.entries('settings.general.item').map(row => row.options.id))
      .toEqual(['transcript-view', 'performance-usage', 'link-opening', 'reasoning-preview-lines', 'composer-enter'])
  ...
  it('mirrors the Host transcript preference into its Settings row', async () => {
    ...
    b.chatSettings.publish({
      status: 'ready', value: { linkOpening: 'sidebar', transcriptView: 'compact', performanceUsage: 'detailed', reasoningPreviewLines: 1 }, revision: 1, writable: true,
    })
