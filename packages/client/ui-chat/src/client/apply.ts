...
export function apply(ctx: Context): void {
  ...
  const reasoningPreview = new ReasoningPreviewPolicy(chatSettings)
  ctx.effect(() => () => { reasoningPreview.dispose() })
  ...
  ctx.slots.inject('conversation.view', () => {
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId): ChatViewInjected => {
        ...
        return {
          hooks: { presentation, reasoningPreviewLines: reasoningPreview.lines },
          keyedHooks: {
