...
Think 行默认保持折叠，并在不展开思维链的情况下暴露实时推理（reasoning）吞吐：折叠形态始终是与标题同行的单行摘要——结算后取首行，该块是流式尾部时取最新的非空行，由单行滚动区横向追到行内末端。当该块正在流式输出且 `ui-conversation.reasoningPreviewLines` 配置为 2–8（整数个换行行数，与 `busyEnter` 一起通过同一 Host 持久化分节存入 `$DSH_HOME/settings.yaml`）时，行内摘要临时换为头部下方的多行窗口，随每个 delta 垂直追随到文本末端；结算后窗口卸载，所有已完成的行回到紧凑的行内形态，因此长 transcript 始终保持单行 Think 行。展开该行会移除任一形态的预览，让完整推理进入普通页面流，因此页面阅读不会与内部跟随器争夺滚动（[决策](../../../.agents/notes/implemented/feature/2026-08-17-configurable-reasoning-preview-lines.md)、[尾随滚动沿革](../../../.agents/notes/implemented/feature/2026-08-02-web-thinking-tail-scroll.md)）。
